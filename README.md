...
- 添加工具页统一支持本地 exe、msi、zip、lnk、bat、cmd、ps1、网页链接和手动收纳。
...
- 工具目录右侧可以根据已选工具分析使用场景，推荐最多 5 个 GitHub 开源项目。
...
- 工具源健康检查会单独探测 winget 包、GitHub Release 资产和直链下载是否仍可用。
- 异常来源可以从更新中心直接交给 AI 修复，并在修复后重新检查。
- 每个工具会显示更新策略：winget 精确更新、重装刷新、手动更新或不由 WinKitBox 更新。

### 日志与操作历史

- 日志中心单独展示实时输出、操作历史和 AI 日志。
...
- 独立记事本页位于日志中心前面，可以创建多个本地文本记录。
- 记事本以类似工具卡片的网格展示，适合保存装机备注、命令片段、链接和个人清单。
- 内容保存在本机浏览器存储中，不会跟随安装脚本执行。

### Windows 环境体检

- 查看系统版本、CPU、内存、硬盘、显卡、网卡等本机信息。
- 检查 winget、Windows PowerShell、.NET Desktop Runtime、VC++ 运行库、Windows 长路径、UTF-8 Beta。
- 可修复项会显示明确按钮；需要重启、安装器或人工确认的项目不会静默修改系统。
...
- 项目可以直接打开 GitHub，或点击 AI 添加一键生成工具配置并加入工具箱。

### 配置备份与界面整理

- 每次打开应用会自动创建本机配置快照，也可以在设置页手动创建和恢复最近备份。
- 工具列表支持拖拽分类、自定义分类排序、已选工具批量移动分类。
- 内置多套浅色二次元图片主题，页面顶部栏、统计卡和侧栏会跟随当前主题保持可读。
...
```text
src/App.tsx              主界面、工具目录、本机页、更新中心、记事本、设置页、日志中心 UI
src/DiscoverView.tsx     GitHub 榜单和 AI 推荐
src/AddToolView.tsx      本地文件、链接 AI 和手动工具添加入口
src/core/                目录数据、安装计划、AI 工具、网络、更新、日志和来源健康模型
...
发布时请确保 GitHub Release tag 与 `package.json` 版本一致，例如 `v0.6.2`。
