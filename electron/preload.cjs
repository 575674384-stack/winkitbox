...
contextBridge.exposeInMainWorld("winKitBox", {
  ...
  getConfigBackups: () => ipcRenderer.invoke("config-backups-get"),
  createConfigBackup: () => ipcRenderer.invoke("config-backup-create"),
  restoreConfigBackup: (fileName) => ipcRenderer.invoke("config-backup-restore", fileName),
  getActivityLog: () => ipcRenderer.invoke("activity-log-get"),
  addActivityLog: (entry) => ipcRenderer.invoke("activity-log-add", entry),
  clearActivityLog: () => ipcRenderer.invoke("activity-log-clear"),
  getAiLog: () => ipcRenderer.invoke("ai-log-get"),
  addAiLog: (entry) => ipcRenderer.invoke("ai-log-add", entry),
  clearAiLog: () => ipcRenderer.invoke("ai-log-clear"),
  ...
  selectLocalPackage: (currentPath) => ipcRenderer.invoke("select-local-package", currentPath),
  selectLocalFile: (currentPath) => ipcRenderer.invoke("select-local-file", currentPath),
  ...
  checkToolUpdates: (descriptors) => ipcRenderer.invoke("check-tool-updates", descriptors),
  checkToolSources: (descriptors) => ipcRenderer.invoke("check-tool-sources", descriptors),
  ...
  recommendAiRepos: (request) => ipcRenderer.invoke("ai-recommend-repos", request),
  fixAiTool: (request) => ipcRenderer.invoke("ai-fix-tool", request),
  analyzeLocalFile: (request) => ipcRenderer.invoke("ai-analyze-local-file", request),
