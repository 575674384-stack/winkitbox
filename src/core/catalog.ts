...
const baseTools: Tool[] = [
  ...
  },
  {
    ...
    customUninstallCommand: [
      ...
      "  $attempts = @(",
      "    @('--id', 'qarmin.czkawka.gui', '--source', 'winget', '--exact', '--disable-interactivity', '--accept-source-agreements', '--silent', '--force', '--purge'),",
      "    @('--id', 'qarmin.czkawka.gui', '--exact', '--disable-interactivity', '--accept-source-agreements', '--force', '--purge'),",
      "    @('--id', 'qarmin.czkawka', '--source', 'winget', '--exact', '--disable-interactivity', '--accept-source-agreements', '--silent', '--force', '--purge'),",
      "    @('--id', 'qarmin.czkawka', '--exact', '--disable-interactivity', '--accept-source-agreements', '--force', '--purge'),",
      "    @('--name', 'Czkawka', '--source', 'winget', '--exact', '--disable-interactivity', '--accept-source-agreements', '--force', '--purge')",
      "  )",
      "  foreach ($wingetArgs in $attempts) {",
      "    & winget uninstall @wingetArgs | Out-Host",
      "    if ($LASTEXITCODE -eq 0) { $global:LASTEXITCODE = 0; return }",
      "  }",
      "  $installed = $false",
      "  foreach ($id in @('qarmin.czkawka.gui', 'qarmin.czkawka')) {",
      ...
      "  }",
      "  if (-not $installed) { $global:LASTEXITCODE = 0; return }",
      "  throw 'WinKitBox uninstall failed: Czkawka'",
      "}"
    ].join("\n"),
    launch: {
